...
export class CdpModal {
    ...
    defaultConfig: CdpModalConfig = {
        classList: {
            host: "absolute h-full w-full grid auto-rows-min place-content-center left-0 top-0 bg-white bg-opacity-80 backdrop-filter"
        },
        animation: {
    ...
    componentWillLoad() {
        ...
        this.computedDisplay = this._config.initalDisplay
    }
    render() {
        const { classList } = this._config;
        const hostClass = classList.host + (this.computedDisplay ? "" : " !hidden")
        return (
            <Host class={hostClass} onClick={() => this.close()}>
                <div class={this.animeClass}><slot></slot></div>
            </Host>
        )
    }
}